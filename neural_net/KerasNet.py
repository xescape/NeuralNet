...
def trainFakeLasso(data, meta):
    ...
    print(
        "lasso model r2 = {0}".format(model.score(data, meta.reshape((meta.shape[0]))))
    )
    coefs = np.abs(model.coef_)
    sorted_idx = sorted(range(coefs.shape[0]), key=lambda x: coefs[x], reverse=True)
...
def makeModel(input_dim):
    ...
    model.add(
        layers.Dense(
            32, input_dim=input_dim, activation="relu", kernel_initializer="normal"
        )
    )
...
def trainModel(data_train, data_test, meta_train):
    ...
    model.fit(
        data_train, meta_train, epochs=1000, batch_size=100, shuffle=True, verbose=0
    )
...
def makeAEModel(input_dim):
    ...
    model.compile(optimizer=optimizers.Adam(0.005), loss="mse", metrics=["mae"])
...
def trainAEModel(data_train, data_test, meta_train):
    ...
    # train autoencoder
    autoencoder.fit(
        all_data, all_data, epochs=500, batch_size=100, shuffle=True, verbose=0
    )
    aemodel = makeAEModel(32)
    aemodel.fit(
        encoder.predict(data_train),
        meta_train,
        epochs=500,
        batch_size=100,
        shuffle=True,
...
def makeConvModel(input_dim):
    ...
    model.add(
        layers.Conv1D(
            filters=8, kernel_size=4, activation="relu", input_shape=(input_dim, 1)
        )
    )
...
def trainConvModel(data_train, data_test, meta_train):
    ...
    model.fit(
        data_train.reshape(data_train.shape[0], -1, 1),
        meta_train,
        epochs=1000,
        batch_size=100,
        shuffle=True,
...
def write_result(meta_test, result, out_path):
    with open(out_path, "w") as output:
        ...
        main_text = [
            ...
            for pair in zip(list(meta_test.reshape((-1,))), list(result.reshape((-1,))))
        ]
        output.write("\n".join(main_text))
