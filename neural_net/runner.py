...
def getHeader(name, out_path, err_path):
    h = [
        ...
        "#SBATCH --time=2:00:00",
        "#SBATCH --job-name {0}".format(name),
        "#SBATCH --output={0}".format(out_path),
        "#SBATCH --error={0}".format(err_path),
        "",
...
def getMods():
    m = ["module load anaconda3/2018.12", "source activate tensorflow", ""]
...
def makeScriptAndRun(script_path, root, p, mode):
    ...
    if mode == "local" or mode == "calypto":
        command = "python3 {script} -i {input} -o {output} -l {layers} -d {dropout} --local".format(
            script=script_path,
            input=root,
            output=folder,
            layers=" ".join([str(x) for x in p[0]]),
            dropout=p[1],
        )
        ...
        command = (
            "python3 {script} -i {input} -o {output} -l {layers} -d {dropout}".format(
                script=script_path,
                input=root,
                output=folder,
                layers=" ".join([str(x) for x in p[0]]),
...
if __name__ == "__main__":
    ...
    if mode == "local":
        script = (
            "/d/workspace/NeuralNet/KerasNet.py"  # location of the nn script itself
        )
