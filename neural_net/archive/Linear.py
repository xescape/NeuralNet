...
def newData(popnet_dir, meta_path, key_path, data_path, label_path):
    ...
    col_labels = data.apply(
        lambda row: concat_chr_pos(row["Chromosome"], row["Position"]), axis=1
    )
...
def getData(data_path, label_path):
    ...
    max = int(
        data.max().max() + 2
    )  # it's hard to account for grey and black so I'm just going to manually add 2. shouldn't be a big deal anyways.
...
def main():
    ...
    if log_path.is_file():
        os.rename(
            log_path,
            old_logs / "nn_log.{0}.txt".format(int(datetime.timestamp(datetime.now()))),
        )
...
def trainAndTest(data_path, label_path, i):
    ...
    print(
        "train score = {0} test score = {1}".format(str(train_score), str(test_score))
    )
