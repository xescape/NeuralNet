...
def createFactors(n_factors, p_regulatory):
    ...
    def createBaseFactor(n):
        return {"id": genID(), "type": "base"}

    def createRegulatoryFactor(target):
        return {"id": genID(), "type": "reg", "target": target["id"]}
    ...
    reg_factors = list(
        map(
            createRegulatoryFactor,
            [random.choice(base_factors) for x in range(n_regulatory)],
        )
    )
...
def annotateFactors(base_factors, regulatory_factors, paintings):
    def helper(factors, eff_fun):
        for factor in factors:
            ...
            pos_samples = random.sample(
                range(n_samples), int(n_samples * percent_allele)
            )
            paintings[pos_samples, pos] = np.random.choice(
                factor["alleles"][1:], n_samples, replace=True
            ).reshape(n_samples, 1)[pos_samples, 0]

    n_samples, n_positions = paintings.shape
    percent_allele = 0.5

    pos_list = random.sample(
        range(n_positions), len(base_factors) + len(regulatory_factors)
    )
    itr = iter(pos_list)
    ...
    reg_eff = 2
    helper(
        regulatory_factors, lambda x: np.float_power(reg_eff, random.choice([1, -1]))
    )
...
if __name__ == "__main__":
    ...
    base_factors, regulatory_factors = annotateFactors(
        base_factors, regulatory_factors, chr_paintings
    )
    # calculate phenotypes
    phenotypes = starmap(
        calculatePhenotype,
        [(base_factors, regulatory_factors, sample) for sample in chr_paintings],
    )
